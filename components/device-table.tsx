...
const columns: ColumnDef<z.infer<typeof schema>>[] = [
	{
		// PENDING: ADD DEVICE ID AS SECOND LINE
		accessorKey: 'device',
		header: 'Dispositivo',
		cell: ({ row }) => {
	...
	},
	{
		accessorKey: 'isTouch',
		header: 'Es táctil?',
		cell: ({ row }) => (row.original.isTouch ? <IconCircleCheck /> : <IconCircleX />),
	},
	{
		accessorKey: 'registered',
		header: 'Fecha de registro',
		cell: ({ row }) => new Date(row.original.registered.seconds * 1000).toDateString(),
	},
	{
...
export function DeviceTable({ data: initialData }: { data: z.infer<typeof schema>[] }) {
								...
								<TableRow key={headerGroup.id}>
									{headerGroup.headers.map(header => {
										return (
											<TableHead
												key={header.id}
												colSpan={header.index === 0 ? 2 : header.colSpan}
											>
